package com.example.myshopping.widget;
...
public class BannerPager extends RelativeLayout implements View.OnClickListener {
    ...
    // 设置广告图片列表
    public void setImage(List<Integer> imageList, boolean flag) {
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (Math.abs(y) > height || Math.abs(x) > width) {
                    getParent().requestDisallowInterceptTouchEvent(false);
                } else {
    ...
    private class ImageAdapater extends PagerAdapter {
        ...
        // 获取页面项的个数
        public int getCount() {
//            Log.d(TAG, "getCount: " + mViewList.size());
